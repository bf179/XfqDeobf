...
3. 使用 Frida 注入目标宿主进程，使用 `Java.use` 调用 QAuxiliary 的 Frida 入口点 `com.fanqie.xfqdeobf.loader.sbl.frida.FridaInjectEntry#entry1(String)` 。
   其中 entry1 方法需要 modulePath 作为参数，需要传入模块 apk 路径，如 `/data/app/com.fanqie.xfqdeobf-1/base.apk`。
...
Java.perform(() => {
    ...
    console.log("ctx:", ctx);
    const ai = ctx.getPackageManager().getPackageInfo("com.fanqie.xfqdeobf", 0).applicationInfo.value;
    ...
    // load com.fanqie.xfqdeobf.loader.sbl.frida.FridaInjectEntry
    const FridaInjectEntry = cl.loadClass("com.fanqie.xfqdeobf.loader.sbl.frida.FridaInjectEntry");
    console.log("FridaInjectEntry:", FridaInjectEntry);
    // com.fanqie.xfqdeobf.loader.sbl.frida.FridaInjectEntry#entry1(String modulePath)
