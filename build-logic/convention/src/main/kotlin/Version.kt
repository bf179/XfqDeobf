...
object Version {
    ...
    fun getNdkVersion(project: Project): String {
        val prop = getLocalProperty(project, "xfqdeobf.override.ndk.version")
        val env = getEnvVariable("QAUXV_OVERRIDE_NDK_VERSION")
        if (!prop.isNullOrEmpty() && !env.isNullOrEmpty()) {
            throw IllegalStateException("Cannot set both QAUXV_OVERRIDE_NDK_VERSION and xfqdeobf.override.ndk.version")
        }
        return prop ?: env ?: defaultNdkVersion
    }

    fun getCMakeVersion(project: Project): String {
        val prop = getLocalProperty(project, "xfqdeobf.override.cmake.version")
        val env = getEnvVariable("QAUXV_OVERRIDE_CMAKE_VERSION")
        if (!prop.isNullOrEmpty() && !env.isNullOrEmpty()) {
            throw IllegalStateException("Cannot set both QAUXV_OVERRIDE_CMAKE_VERSION and xfqdeobf.override.cmake.version")
        }
        return prop ?: env ?: defaultCMakeVersion
    }

    fun getNinjaPathOrNull(project: Project): String? {
        val prop = getLocalProperty(project, "xfqdeobf.override.ninja.path")
        val env = getEnvVariable("QAUXV_OVERRIDE_NINJA_PATH")
        if (!prop.isNullOrEmpty() && !env.isNullOrEmpty()) {
            throw IllegalStateException("Cannot set both QAUXV_OVERRIDE_NINJA_PATH and xfqdeobf.override.ninja.path")
        }
        return prop ?: env
    }
